...
a1 <- SimulateMM1(lambda = 10, mu = 15)
a$customers

prumer_prazdny <- function(a){
n = 0
simsum <- 0
for (i in 1:nrow(a$states)){
  if (a$states$state[i] == 0 | a$states$state[i] == 1) {
    simsum <- simsum + a$states$duration[i]
    n = n + 1
  }
}
return(simsum/n)
